BeforeAll {
    ...
    . $PSScriptRoot/../Private/Web/Get-ElmXtermPage.ps1
}

Describe 'Get-ElmXtermPage' {

    Context 'HTML structure' {
        BeforeAll {
            $script:html = Get-ElmXtermPage -Port 8080 -Cols 220 -Rows 50 -Title 'Test App'
        }
    ...
    Context 'Terminal dimensions' {
        It 'Should embed the correct cols in Terminal constructor' {
            $html = Get-ElmXtermPage -Port 8080 -Cols 160 -Rows 40
    ...
    Context 'JavaScript event handler syntax' {
        BeforeAll {
            $script:html = Get-ElmXtermPage -Port 8080 -Cols 220 -Rows 50
        }
    ...
    Context 'Defaults' {
        It 'Should use "Elm TUI" as default title' {
            $html = Get-ElmXtermPage -Port 8080
            $html | Should -Match '<title>Elm TUI</title>'
        }
    }
}
