BeforeAll {
    # Stubs for dependencies called by New-TeaWebSocketDriver

    $script:XtermJs       = '/* stub */'
    $script:XtermAddonFit = '/* stub */'
    $script:XtermCss      = ''

    function Get-TeaXtermPage {
    ...
    . $PSScriptRoot/../Public/Drivers/New-TeaWebSocketDriver.ps1
}

Describe 'New-TeaWebSocketDriver' {

    Context 'Return shape' {
        BeforeAll {
            $script:driver = New-TeaWebSocketDriver -Port 8080 -Width 220 -Height 50
        }
    ...
    Context 'InputQueue type' {
        It 'Should expose a ConcurrentQueue as InputQueue' {
            $driver = New-TeaWebSocketDriver -Port 8081
