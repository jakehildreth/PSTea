BeforeAll {
    . $PSScriptRoot/../Public/View/New-ElmProgressBar.ps1
}

Describe 'New-ElmProgressBar' -Tag 'Unit', 'P9' {

    Context 'Return value structure' {
        It 'Should return a Text node' {
            $bar = New-ElmProgressBar -Value 0.5
    ...
    Context 'Bar content format' {
        It 'Should start with [ and end with ]' {
            $bar = New-ElmProgressBar -Value 0.5 -Width 20
        ...
        It 'Should be all filled at Value 1.0' {
            $bar = New-ElmProgressBar -Value 1.0 -Width 12
            # inner = 10 chars, all '#'
            $bar.Content | Should -Be ('[' + ('#' * 10) + ']')
        }

        It 'Should be all empty at Value 0.0' {
            $bar = New-ElmProgressBar -Value 0.0 -Width 12
            $bar.Content | Should -Be ('[' + ('-' * 10) + ']')
        }

        It 'Should fill approximately half at Value 0.5' {
            $bar = New-ElmProgressBar -Value 0.5 -Width 22
    ...
    Context 'Percent parameter set' {
        It 'Should accept -Percent 100 and fill bar completely' {
            $bar = New-ElmProgressBar -Percent 100 -Width 12
            $bar.Content | Should -Be ('[' + ('#' * 10) + ']')
        }

        It 'Should accept -Percent 0 and show empty bar' {
            $bar = New-ElmProgressBar -Percent 0 -Width 12
            $bar.Content | Should -Be ('[' + ('-' * 10) + ']')
        }

        It 'Should accept -Percent 50' {
            $bar = New-ElmProgressBar -Percent 50 -Width 22
            $bar.Content | Should -Be ('[' + ('#' * 10) + ('-' * 10) + ']')
        }
    }
    ...
    Context 'Value clamping' {
        It 'Should clamp Value above 1.0 to full bar' {
            $bar = New-ElmProgressBar -Value 2.5 -Width 12
            $bar.Content | Should -Be ('[' + ('#' * 10) + ']')
        }

        It 'Should clamp Value below 0.0 to empty bar' {
            $bar = New-ElmProgressBar -Value -0.5 -Width 12
            $bar.Content | Should -Be ('[' + ('-' * 10) + ']')
        }
    }
    ...
    Context 'Custom characters' {
        It 'Should use custom FilledChar' {
            $bar = New-ElmProgressBar -Value 1.0 -Width 7 -FilledChar '='
            $bar.Content | Should -Be ('[' + ('=' * 5) + ']')
        }

        It 'Should use custom EmptyChar' {
            $bar = New-ElmProgressBar -Value 0.0 -Width 7 -EmptyChar '.' 
            $bar.Content | Should -Be ('[' + ('.' * 5) + ']')
        }
    }
    ...
    Context 'Style passthrough' {
        It 'Should pass Style to the Text node' {
            ...
            $bar   = New-ElmProgressBar -Value 0.5 -Style $style
    ...
    Context 'Error handling' {
        It 'Should throw when Width is below minimum (4)' {
            { New-ElmProgressBar -Value 0.5 -Width 3 } | Should -Throw
        }
    }
}
