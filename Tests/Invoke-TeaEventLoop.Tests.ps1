BeforeAll {
    ...
    function Measure-TeaViewTree { param($Root, $TermWidth, $TermHeight) return $Root }
    function Compare-TeaViewTree { param($OldTree, $NewTree) return @() }
    function ConvertTo-AnsiOutput { param($Root) return '' }
    function ConvertTo-AnsiPatch { param($Patches) return '' }

    . $PSScriptRoot/../Private/Runtime/Invoke-TeaEventLoop.ps1
}

Describe 'Invoke-TeaEventLoop' {
    Context 'Quit command' {
        It 'Should stop when UpdateFn returns a Quit command' {
            ...
            $viewFn = { param($model) New-TeaText -Content 'test' }
            $queue = [System.Collections.Concurrent.ConcurrentQueue[object]]::new()
            $queue.Enqueue('any')
            { Invoke-TeaEventLoop -InitialModel ([PSCustomObject]@{}) -UpdateFn $updateFn -ViewFn $viewFn -InputQueue $queue } | Should -Not -Throw
        }
        ...
        It 'Should return the final model on Quit' {
            ...
            $viewFn = { param($model) New-TeaText -Content 'x' }
            $queue = [System.Collections.Concurrent.ConcurrentQueue[object]]::new()
            $queue.Enqueue('hello')
            $queue.Enqueue('Quit')
            $result = Invoke-TeaEventLoop -InitialModel ([PSCustomObject]@{ Value = '' }) -UpdateFn $updateFn -ViewFn $viewFn -InputQueue $queue
        ...
        It 'Should capture the model returned by the Quit update' {
            ...
            $result = Invoke-TeaEventLoop -InitialModel ([PSCustomObject]@{ Last = '' }) -UpdateFn $updateFn -ViewFn $viewFn -InputQueue $queue
    ...
    Context 'Message processing' {
        It 'Should accumulate model changes across multiple messages' {
            ...
            $viewFn = { param($model) New-TeaText -Content "$($model.Count)" }
            $queue = [System.Collections.Concurrent.ConcurrentQueue[object]]::new()
            $queue.Enqueue('Inc')
            $queue.Enqueue('Inc')
            $queue.Enqueue('Inc')
            $queue.Enqueue('Quit')
            $result = Invoke-TeaEventLoop -InitialModel ([PSCustomObject]@{ Count = 0 }) -UpdateFn $updateFn -ViewFn $viewFn -InputQueue $queue
        ...
        It 'Should pass the current model to the View function after each update' {
            ...
            $viewFn = { param($model) New-TeaText -Content "label:$($model.Label)" }
            $queue = [System.Collections.Concurrent.ConcurrentQueue[object]]::new()
            $queue.Enqueue('alpha')
            $queue.Enqueue('Quit')
            # If ViewFn were not called, we would get an error from Invoke-TeaView; instead it completes
            { Invoke-TeaEventLoop -InitialModel ([PSCustomObject]@{ Label = '' }) -UpdateFn $updateFn -ViewFn $viewFn -InputQueue $queue } | Should -Not -Throw
        }
    }
}
