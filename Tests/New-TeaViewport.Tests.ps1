BeforeAll {
    . $PSScriptRoot/../Public/View/New-TeaViewport.ps1
}

Describe 'New-TeaViewport' -Tag 'Unit', 'P9' {

    Context 'Return value structure' {
        It 'Should return a Box node' {
            $vp = New-TeaViewport -Lines @('a', 'b', 'c') -MaxVisible 5
    ...
    Context 'Basic windowing' {
        ...
        It 'Should show correct content at offset 0' {
            ...
            $vp    = New-TeaViewport -Lines $lines -ScrollOffset 0 -MaxVisible 2
    ...
    Context 'ScrollOffset clamping' {
        It 'Should clamp ScrollOffset below 0 to 0' {
            $lines = @('A', 'B', 'C')
            $vp    = New-TeaViewport -Lines $lines -ScrollOffset -5 -MaxVisible 3
            $vp.Children[0].Content | Should -Be 'A'
        }

        It 'Should clamp ScrollOffset so window does not exceed end of lines' {
            $lines = @('A', 'B', 'C')
            $vp    = New-TeaViewport -Lines $lines -ScrollOffset 100 -MaxVisible 3
    ...
    Context 'Empty lines' {
        It 'Should return a single-child Box when Lines is empty' {
            $vp = New-TeaViewport -Lines @() -MaxVisible 5
            $vp.Type           | Should -Be 'Box'
    ...
    Context 'Style passthrough' {
        It 'Should apply Style to each visible line' {
            ...
            $lines = @('A', 'B', 'C')
            $vp    = New-TeaViewport -Lines $lines -MaxVisible 3 -Style $style
        ...
        It 'Should have null Style on children when Style omitted' {
            $lines = @('A', 'B')
            $vp    = New-TeaViewport -Lines $lines -MaxVisible 2
