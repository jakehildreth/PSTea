...
Describe 'New-TeaTimerSub' -Tag 'Unit', 'P6' {
    ...
    Context 'Various intervals' {
        It 'Should accept 1ms (minimum)' {
            $sub = New-TeaTimerSub -IntervalMs 1 -Handler { 'x' }
            $sub.IntervalMs | Should -Be 1
        }

        It 'Should accept 500ms' {
            $sub = New-TeaTimerSub -IntervalMs 500 -Handler { 'x' }
            $sub.IntervalMs | Should -Be 500
        }

        It 'Should accept 60000ms (1 minute)' {
            $sub = New-TeaTimerSub -IntervalMs 60000 -Handler { 'x' }
    ...
    Context 'Handler invocation' {
        ...
        It 'Should invoke handler that returns a PSCustomObject' {
            $sub = New-TeaTimerSub -IntervalMs 200 -Handler {
        ...
        It 'Should allow handler that returns null' {
            $sub = New-TeaTimerSub -IntervalMs 100 -Handler { $null }
            $msg = & $sub.Handler
    ...
    Context 'Error handling' {
        It 'Should throw when IntervalMs is 0' {
            { New-TeaTimerSub -IntervalMs 0 -Handler { 'x' } } | Should -Throw
        }

        It 'Should throw when IntervalMs is negative' {
            { New-TeaTimerSub -IntervalMs -1 -Handler { 'x' } } | Should -Throw
        }
    }
}
