...
$script:ElmWebDebugLog = '/tmp/elm-web-debug.log'

function Write-ElmWebDebug {
    param([string]$Message)
    $ts = [datetime]::Now.ToString('HH:mm:ss.fff')
    Add-Content -Path $script:ElmWebDebugLog -Value "[$ts] $Message" -ErrorAction SilentlyContinue
}
...
function Invoke-ElmWebSocketListener {
    ...
    .DESCRIPTION
        ...
        Debug output is appended to /tmp/elm-web-debug.log.
    ...
    try { Remove-Item -Path $script:ElmWebDebugLog -Force -ErrorAction SilentlyContinue } catch {}
    Write-ElmWebDebug "Invoke-ElmWebSocketListener starting on port $Port"

    $listener = [System.Net.HttpListener]::new()
    $listener.Prefixes.Add("http://localhost:$Port/")
    $listener.Start()
    Write-ElmWebDebug "HttpListener started"
    ...
    Write-ElmWebDebug "VT parser path: $vtParserPath (exists=$(Test-Path $vtParserPath))"
    ...
    [void]$sendPs.AddArgument($script:ElmWebDebugLog)
    $sendAr = $sendPs.BeginInvoke()
    Write-ElmWebDebug "Send runspace started"
    ...
    [void]$acceptPs.AddArgument($script:ElmWebDebugLog)
    $acceptAr = $acceptPs.BeginInvoke()
    Write-ElmWebDebug "Accept runspace started"
