function Get-TeaXtermPage {
    ...
    .DESCRIPTION
        Generates the HTML served at GET / by Invoke-TeaWebSocketListener. The page:
        ...
        at module import time (set by PSTea.psm1 from Private/Web/).

    .PARAMETER Port
        TCP port the WebSocket server is listening on. Used in the ws:// URL.

    .PARAMETER Title
        HTML page title. Defaults to "PSTea TUI".

    .PARAMETER Cols
        Terminal width in columns. Should match -Width passed to Start-TeaWebServer.

    .PARAMETER Rows
        Terminal height in rows. Should match -Height passed to Start-TeaWebServer.

    .OUTPUTS
        string - Complete HTML document as a string.

    .EXAMPLE
        $html = Get-TeaXtermPage -Port 8080 -Cols 220 -Rows 50
