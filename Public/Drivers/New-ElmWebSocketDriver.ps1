function New-ElmWebSocketDriver {
    <#
    .SYNOPSIS
        Creates an Elm driver that pipes a TUI application through a WebSocket/xterm.js interface.
    ...
    .DESCRIPTION
        Sets up:
          ...
          4. An HttpListener + WebSocket accept loop via Invoke-ElmWebSocketListener.

        Returns a PSCustomObject with the shape expected by Invoke-ElmEventLoop:
            { InputQueue, OutputSink, Stop }

        InputQueue  - ConcurrentQueue[PSCustomObject] to pass to Invoke-ElmEventLoop -InputQueue.
        OutputSink  - scriptblock { param($s) } to pass to Invoke-ElmEventLoop -OutputSink.
    ...
    .PARAMETER Width
        Terminal width in columns. Passed to Get-ElmXtermPage and used for TerminalWidth in
        Invoke-ElmEventLoop. Defaults to 220.

    .PARAMETER Height
        Terminal height in rows. Passed to Get-ElmXtermPage and used for TerminalHeight in
        Invoke-ElmEventLoop. Defaults to 50.

    .PARAMETER Title
        Browser page title. Defaults to "Elm TUI".

    .OUTPUTS
        PSCustomObject { InputQueue, OutputSink, Stop }

    .EXAMPLE
        $driver = New-ElmWebSocketDriver -Port 8080 -Width 220 -Height 50
        Invoke-ElmEventLoop -InitFn $init -UpdateFn $update -ViewFn $view `
    ...
    # OutputSink for Invoke-ElmEventLoop: captures $outputQueue via closure.
    ...
    $htmlContent = Get-ElmXtermPage -Port $Port -Cols $Width -Rows $Height -Title $Title

    # Start the HTTP/WebSocket listener (returns { Listener, SharedState, Stop }).
    $wsServer = Invoke-ElmWebSocketListener -Port $Port -InputQueue $inputQueue `
