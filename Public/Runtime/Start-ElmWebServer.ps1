function Start-ElmWebServer {
    <#
    .SYNOPSIS
        Starts an Elm TUI application served in a browser via WebSocket and xterm.js.

    .DESCRIPTION
        Runs an Elm MVU application in a browser using a self-hosted HTTP/WebSocket server.
        ...
        This function bypasses Start-ElmProgram to avoid PTY dimension validation,
    ...
    .PARAMETER SubscriptionFn
        ...
        created by New-ElmKeySub and/or New-ElmTimerSub.
    ...
    .PARAMETER Title
        Browser tab title. Defaults to "Elm TUI".

    .EXAMPLE
        Start-ElmWebServer -InitFn { [PSCustomObject]@{ Model = @{ Count = 0 }; Cmd = $null } } `
    ...
    # Optional tick loop (same pattern as Start-ElmProgram)
    $tickLoop = $null
    if ($TickMs -gt 0) {
